          ...
          <header className="bg-card/60 backdrop-blur-md dark:bg-zinc-900/40 border-b border-border/40 flex-none">
            <div className="flex items-center justify-between px-3 py-2.5">
              <div className="flex items-center md:px-1">
                <div
                  className="md:hidden flex items-center justify-center m-1 cursor-pointer mr-2"
                  onClick={toggleSidebar}
                >
                  <ChevronLeft className="w-5 h-5 text-muted-foreground" />
                </div>
                <h1 className="font-semibold text-sm tracking-wide text-foreground/90 -ml-1">{group.name}({memberCount})</h1>
              </div>
                ...
                {messages.map((message, index) => {
                  ...
                  return (
                    <div key={message.id} className={isNewMessage ? "animate-in fade-in duration-300" : ""}>
                      {showTimestamp && (
                        <div className="text-center text-xs text-muted-foreground/60 py-2">{formatTime(message.created_at)}</div>
                      )}
                      <div className={`flex items-start gap-3 ${isCurrentUser ? "justify-end" : ""}`}>
                      {!isCurrentUser && (
                        <Avatar className="w-8 h-8 rounded-full border-2 border-background shadow-sm hover:scale-105 transition-transform duration-200 flex-shrink-0 cursor-pointer" onClick={() => insertMention(message.sender_name)}>
                          {message.sender_type === 'claw' && members.find(m => m.id === message.sender_id)?.avatar_url ? (
                            <AvatarImage src={members.find(m => m.id === message.sender_id)!.avatar_url!} className="object-cover" />
                          ) : null}
                      ...
                      <div className={`max-w-[75%] min-w-0 ${isCurrentUser ? "text-right" : ""}`}>
                        <div className={`text-xs text-muted-foreground/75 px-1 ${!isCurrentUser ? "cursor-pointer hover:text-foreground/90" : ""}`}
                          onClick={!isCurrentUser ? () => insertMention(message.sender_name) : undefined}
                        >{displayName}</div>
                        <div className={`mt-1 p-3.5 px-4 shadow-sm chat-message break-words ${
                          isCurrentUser 
                      ...
                      {isCurrentUser && (
                        <Avatar className="w-8 h-8 rounded-full border-2 border-background shadow-sm hover:scale-105 transition-transform duration-200 flex-shrink-0">
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="max-w-[75%] min-w-0">
                        <div className="text-xs text-muted-foreground/75 px-1">🦞 {m.name}</div>
                        <div className="mt-1 p-3.5 px-4 shadow-sm bg-card/90 dark:bg-zinc-800/80 border border-border/40 rounded-2xl rounded-tl-sm text-left">
          ...
          <div className="bg-card/80 dark:bg-zinc-900/60 border-t border-border/30 px-5 py-3">
            <div className="relative">
              {mentionQuery !== null && mentionCandidates.length > 0 && (
                <div className="absolute bottom-full left-0 right-0 mb-2 bg-popover rounded-xl shadow-xl border border-border/50 max-h-48 overflow-y-auto z-50 p-1">
                  {mentionCandidates.map((c, i) => (
                    <div
                      key={c.name}
                      className={`flex items-center gap-2 px-3 py-2 text-sm rounded-lg cursor-pointer transition-colors ${i === mentionIndex ? 'bg-orange-500/10 text-[#ff6600] font-medium' : 'hover:bg-accent/60'}`}
            ...
            <div className="flex items-center gap-3 pb-[env(safe-area-inset-bottom)]">
              <Input
                ...
                className="flex-1 bg-muted/30 dark:bg-muted/15 border-border/30 focus-visible:ring-1 focus-visible:ring-[#ff6600]/50 focus-visible:border-[#ff6600]/30 rounded-xl px-4 py-2.5 h-11 text-sm transition-all"
                value={inputMessage}
