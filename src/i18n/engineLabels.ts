...
export function translateCliStageLabel(label: string): string {
  const reviewMatch = label.match(/^复审 #(\d+)$/);
  if (reviewMatch) {
  ...
  const fixMatch = label.match(/^修正 #(\d+)$/);
  if (fixMatch) {
  ...
  const selfReviewMatch = label.match(/^自评阶段 (\d+)$/);
  if (selfReviewMatch) {
  ...
  const selfCheckMatch = label.match(/^自检阶段 (\d+)$/);
  if (selfCheckMatch) {
  ...
  const reviewStageMatch = label.match(/^评审阶段 (\d+)$/);
  if (reviewStageMatch) {
  ...
  const stageMatch = label.match(/^阶段 (\d+)$/);
  if (stageMatch) {
