import {
  MAX_ATTACHMENTS_PER_MESSAGE as SESSION_MAX_ATTACHMENTS_PER_MESSAGE,
  type ChatAttachment,
  type ChatAttachmentKind,
} from '@/config/chatSessions';

export const MAX_ATTACHMENTS_PER_MESSAGE = SESSION_MAX_ATTACHMENTS_PER_MESSAGE;
...
export function classifyAttachmentPath(path: string): AttachmentClassification | null {
  ...
  if (documentMime) return { kind: 'document', extension, mimeType: documentMime };
...
export function createChatAttachment(candidate: AttachmentCandidate): ChatAttachment | null {
  ...
  const name = candidate.name?.trim() || basenameFromPath(path);
  const attachment: ChatAttachment = {
    id: fallbackId(),
    kind: classification.kind,
    name,
    path,
    mimeType: candidate.mimeType?.trim() || classification.mimeType,
...
export function formatAttachmentsForHistory(attachments?: ChatAttachment[]): string {
  ...
  return `[附件: ${attachments.map(attachment => `${attachment.name} -> ${attachment.path}`).join('; ')}]`;
}
